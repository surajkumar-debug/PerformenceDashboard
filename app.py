...
<style>
    ...
    * { font-family: 'Inter', sans-serif; }
    ...
    .kpi-card {
        ...
        padding: 20px 24px; text-align: center; transition: transform 0.2s ease;
    }
    ...
    .section-header {
        ...
        font-size: 16px; font-weight: 700; margin: 20px 0 16px 0;
    }
...
# ─────────────────────────────────────────────
# PLOT THEME - NO MARGIN HERE (fix duplicate key bug)
# ─────────────────────────────────────────────
PLOT_LAYOUT = dict(
...
def main():
    ...
    st.markdown(f"""
    <div style="background:linear-gradient(135deg,#1e40af 0%,#0f766e 50%,#7c3aed 100%);
    ...
    with st.sidebar:
        ...
        if auto_refresh:
            if AUTOREFRESH_AVAILABLE:
                ...
                st.markdown('<div class="info-box">✅ Auto-refresh active</div>', unsafe_allow_html=True)
            else:
                st.markdown('<div class="info-box">⚠️ Press F5 to refresh</div>', unsafe_allow_html=True)
        ...
        st.markdown("---")
        if detected['date_cols']:   st.success(f"📅 {', '.join(detected['date_cols'][:2])}")
        if detected['amount_cols']: st.info(f"💰 {', '.join(detected['amount_cols'][:3])}")
        if detected['user_cols']:   st.warning(f"👤 {', '.join(detected['user_cols'][:2])}")
        ...
        if amt_col:
            ...
            with c3:
                ...
                st.markdown(f"""<div class="kpi-card">
                    ...
                    <div class="kpi-delta-neu">all entries</div>
                </div>""", unsafe_allow_html=True)
        ...
        if usr_col:
            with c4:
                st.markdown(f"""<div class="kpi-card">
                    ...
                    <div class="kpi-value">{df[usr_col].nunique()}</div>
                    <div class="kpi-delta-pos">in system</div>
                </div>""", unsafe_allow_html=True)
            with c5:
                st.markdown(f"""<div class="kpi-card">
                    <div class="kpi-title">Active This Month</div>
                    <div class="kpi-value">{safe_nunique(curr_month_df, usr_col)}</div>
                    <div class="kpi-delta-pos">submitting</div>
                </div>""", unsafe_allow_html=True)
        ...
        if detected['cat_cols']:
            ...
            for idx, cc in enumerate(detected['cat_cols'][:4]):
                with cols[idx]:
                    ...
                    fig = px.pie(values=vc.values, names=vc.index, title=cc,
                                 color_discrete_sequence=COLORS, hole=0.4)
                    fig.update_layout(margin=MARGIN_COMPACT, **PLOT_LAYOUT)
                    st.plotly_chart(fig, use_container_width=True)
    ...
    with tab2:
        ...
        if not usr_col:
            ...
            if amt_col:
                agg_dict.update({
                    'Total_Value': (amt_col, 'sum'),
                    'Avg_Value':   (amt_col, 'mean'),
                    'Max_Value':   (amt_col, 'max')
                })
            ...
            with col1:
                if amt_col:
                    fig = px.bar(user_summary.head(15), x='Total_Value', y=usr_col, orientation='h',
                                 title='💰 Total Value by User', color='Total_Value',
                                 color_continuous_scale='Viridis', text='Total_Value')
            ...
            if '_month' in df.columns and amt_col:
                ...
                fig = px.imshow(pivot, aspect='auto', color_continuous_scale='Blues',
                                title='User Performance Heatmap',
    ...
    with tab3:
        ...
        gran = st.radio("Select Time Granularity",
        ...
        period_col  = period_map.get(gran, '_month')
        gran_label  = gran.split(' ')[-1]
        ...
        if period_col not in df.columns:
            ...
            fig.add_trace(go.Bar(x=trend[period_col].astype(str), y=trend['Total'],
                                 name='Total Value', marker_color='#3b82f6', opacity=0.8), row=1, col=1)
            fig.add_trace(go.Scatter(x=trend[period_col].astype(str), y=trend['Total'],
                                     name='Trend', line=dict(color='#f59e0b', width=2, dash='dot'),
                                     mode='lines'), row=1, col=1)
            fig.add_trace(go.Bar(x=trend[period_col].astype(str), y=trend['Count'],
                                 name='Count', marker_color='#10b981', opacity=0.7), row=2, col=1)
            fig.update_layout(title=f"📈 {gran_label} Performance Trend",
                              height=500, margin=MARGIN_DEFAULT, **PLOT_LAYOUT)
            st.plotly_chart(fig, use_container_width=True)
        ...
        if '_weekday' in df.columns:
            ...
            col1, col2 = st.columns(2)
            with col1:
                fig = px.bar(day_data, x='Day', y='Count', title='📊 Submissions by Weekday',
            ...
            with col2:
                if amt_col:
                    ...
                    fig = px.line_polar(day_amt, r='Total', theta='Day', title='🎯 Value Radar by Weekday',
                                        line_close=True, color_discrete_sequence=['#3b82f6'])
                    fig.update_layout(margin=MARGIN_DEFAULT, **PLOT_LAYOUT)
                    st.plotly_chart(fig, use_container_width=True)
    ...
    with tab4:
        ...
        c_cnt = len(curr_month_df)
        p_cnt = len(prev_month_df)
        pct_cnt = ((c_cnt - p_cnt) / abs(p_cnt) * 100) if p_cnt else 0
        with c1:
            st.metric("📝 Entries", f"{c_cnt:,}", f"{c_cnt-p_cnt:+,} ({pct_cnt:+.1f}%)")
        ...
        if amt_col and usr_col and not curr_month_df.empty and not prev_month_df.empty:
            ...
            with col1:
                ...
                fig = px.bar(pd.concat([curr_u, prev_u]), x=usr_col, y=amt_col, color='Period',
                             barmode='group', title='📊 Current vs Last Month',
        ...
        cw_cnt = len(curr_week)
        pw_cnt = len(prev_week)
        ...
        if amt_col:
            ...
            if usr_col:
                with c3:
                    st.metric("👤 Active Users", safe_nunique(curr_week, usr_col),
                              f"{safe_nunique(curr_week, usr_col)-safe_nunique(prev_week, usr_col):+,}")

        if amt_col and usr_col and not curr_week.empty and not prev_week.empty:
            cw_u = curr_week.groupby(usr_col)[amt_col].sum().reset_index()
            cw_u['Period'] = 'Current Week'
            pw_u = prev_week.groupby(usr_col)[amt_col].sum().reset_index()
            pw_u['Period'] = 'Last Week'
            fig = px.bar(pd.concat([cw_u, pw_u]), x=usr_col, y=amt_col, color='Period',
                         barmode='group', title='📊 Current vs Last Week',
    ...
    with tab5:
        ...
        st.download_button("⬇️ Download Filtered CSV", csv_dl, "invoice_data.csv", "text/csv")

    st.markdown("""
    <div style="text-align:center;color:#4a5568;font-size:12px;margin-top:40px;
                padding:16px;border-top:1px solid #2d3250;">
        📊 Invoice Performance Dashboard • Streamlit + Plotly • Live Google Sheets
    </div>""", unsafe_allow_html=True)
